var HolyDays = function () {
  var $ = jQuery;
  var _footer = 'Coding last updated on 11 Sovereignty 180 / 28 January 2024.';
  ...
  function FillHead () {
    ...
    var html =
      ...
      '<div id=printBtnArea><button type=button onclick="print()">Print</button></div>'
      + '<div class="noPrint">'
      + '<div class="SelectorArea">'
      + '<div class="Selectors">'
      + '<div class="YearSelect">Year to Display: <select accesskey=Y id="DesiredYear"></select>'
      + '<button id=goNow class=now type=button>This Year</button>'
      + '<button id=goNext type=button>Next &rarr;</button>'
      + '<button id=goPrev type=button>&larr; Previous</button>'
      + '</div>'
      ...
      + '<span class=LatLong><input id="ShowSunset" type="checkbox" accesskey="U" /><label for="ShowSunset"><i> Show Your Sunset Times</i></label> <span class="help" title="To accurately show when each day begins, your location is needed."></span></span>'
      + '<div class=Clear></div>'
      + '</div>'
      + '<div class=Clear></div>'
      + '<div class=Sunset>'
      + '<div>Latitude: <input id=lat> Longitude: <input id=long> <span title="Nearest location" class="Sunset latLongName"></span>'
      + ' <span class="help" title="Tip: if this is wrong, you can find your location in Google Maps then\ncopy the Latitude and Longitude numbers from the URL address after the @."></span>'
      + ' <button type=button id=getLocation>Guess my Location</button> '
      + '</div>'
      + '<div><i>Your clock is using the ' + timeZoneName + ' zone. Your lat/long location indicates the <span id=geoTime></span> zone.<br>If these do not match, sunset times may be incorrect.</i><span class="help" title="Tip: You device\'s clock and location are also used to determine daylight savings start and end times throughout the year."></span></div>'
      + '</div>'
      + '</div>'

      + '<h2>Year <span class=showYear></span></h2>'
      + '<h2 class="subtitle latLongName"></h2>'
      + '<table class=List id=List><thead>'
      + '<tr>'
      + '<th class="Num ForFeast">Month<br>#</th>'
      + '<th class=Name1>Name</th>'
      + '<th>Date(s)</th>'
      + '<th class=Sunset>Ending<br>Sunset</th>'
      + '<th class=Gregorian>Week Day(s)</th>'
      + '<th class=Gregorian>Gregorian Date(s)</th>' //(<span class=gYear></span>)
      + '<th class="Gregorian ForFeast ForStartTime">Suggested Start Time <select class=Feast id=StartTime><option value=1900>7:00pm</option><option value=1930>7:30pm</option><option value=2000>8:00pm</option></select><span class="help" title="Tip: Select the desired start time to know if the event falls before or after sunset."></span></th>'
      //       + '<th class=Gregorian>Dates</th>'
      + '</tr>'
      + '</thead><tbody>'
      + '</tbody></table>'

    $('#HolyDaysList').html(html);
  ...
  function goYear (delta) {
    ...
    if (delta === 0) {
      ddlYear.val(_nowYear);
    } else {
      var select = ddlYear[0];
  ...
  function FillChoices () {
    if (localStorage.WantFeast == null) {
      ...
      localStorage.ShowSunset = 'true';
    }

    $('#goNow').click(function () { goYear(0) });
    $('#goNext').click(function () { goYear(1) });
    $('#goPrev').click(function () { goYear(-1) });
    ...
    $('#ShowSunset')
      .click(function () {
        ApplyFilters();
        localStorage.ShowSunset = $(this).prop('checked');
      })
    ...
    ddlYear
      .change(function () {
        ...
        $('#goNow').toggleClass('now', _nowYear == year)
        showDatesInYear($('.List'), year);
    ...
    $('.latLongName').text(localStorage.where);
    $('#geoTime').text(localStorage.zone || '(click Guess my Location)');
        ...
        try {
          ...
          xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
              ...
              $('#lat').val(localStorage.lat = _locationLat = loc[0]);
              $('#long').val(localStorage.long = _locationLong = loc[1]);
              $('.latLongName').text(localStorage.where = info.city);
              $('#geoTime').text(localStorage.zone = info.timezone);
              continueAfterGetLocation();
            }
          }
          xhr.open("GET", url, true);
          xhr.timeout = 1000;
          xhr.ontimeout = function () { continueAfterGetLocation(); };
          xhr.send();
  ...
  function continueAfterGetLocation () {
    $('#DesiredYear').trigger('change');
  }
  ...
  function FillYearsList () {
    ...
    var currentBYear = _now.getFullYear() - 1843 + _now.isAfterNawRuz() ? 1 : 0;
    $('#DesiredYear')
      .append(options.join('\n'))
      .val(currentBYear);
  ...
  function prepareDateInfos (bYear) {
    ...
    if (!_monthNames.length) {
      _monthNames.length = 20;
      _monthNames[0] = { en: 'Intercalary', ar: 'Ayyám-i-Há' };
      _dateInfos
        .filter(function (d) { return d.Type === 'M' })
        .forEach(function (d) {
          _monthNames[d.MonthNum] = {
            en: d.NameEn,
            ar: d.NameAr
          }
        });
    ...
    $('#HolyDaysList').removeClass('hasToday')
    if (bNow.y == bYear) {
      $('#HolyDaysList').addClass('hasToday')
      _dateInfos.push(
        {
    ...
    for (var i = 0; i < _dateInfos.length; i++) {
      ...
      if (dateInfo.Type == 'M') {
        rows.push(
          ...
          + '<td class=Name2>' + dateInfo.NameEn + ' (' + dateInfo.NameAr + ')</td>'
        ...
        rows.push(
          ...
          + '<td>' + dateInfo.GDate.getDayNames() + '</td>'
          + '<td class=SpecificDay>' + ShortGregDate(dateInfo.GDate) + ', ' + dateInfo.gYear + '</td>'
          + '<td class="ForStartTime" data-time="' + dateInfo.Time + '" data-gdate="' + dateInfo.GDate.getTime() + '"></td>'
          + '</tr>'
        );
      }
        ...
        rows.push(
          ...
          + '<td>' + dateInfo.GDate.getDayOfWeek() + '</td>'
          + '<td class=SpecificDay>' + _gMonthNames[dateInfo.GDate.getMonth()] + ' ' + dateInfo.GDate.getDate() + ', ' + dateInfo.gYear + '</td>'
          + '<td class=ForFeast></td>'
          + '</tr>'
        );
      }
  ...
  function ShortBadi (bMonthDay) {
    // show this date
    return _monthNames[bMonthDay.m].en + ' ' + bMonthDay.d;
  ...
  var getBDate = function (d) {
    ...
    if (typeof SunCalc != 'undefined' && _locationLat != 0 && _locationLong != 0) {
    ...
    $('td.ForStartTime').each(function (i, dom) {
      ...
      if (d && targetTime) {
        ...
        if (targetTime == 'SS+2') {
          ...
          var isToday = el.closest('tr').hasClass('Today');
          ...
          if (isToday) {
            el.html('Time now: ' + ShortGregDateNoEve(date));
          }
          else {
            var times = SunCalc.getTimes(date, _locationLat, _locationLong);
            if (times.sunset.getTime() < date.getTime()) {
              // after sunset
              date.setHours(date.getHours() - 24);
              el.html(ShortGregDateNoEve(date) + ' (eve)' + asterisk);
              // Time now:
            }
            else {
              el.html(ShortGregDateNoEve(date) + asterisk);
            }
          }
        }
